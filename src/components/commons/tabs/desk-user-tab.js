  ...
  (
    <div className="flex items-center w-full text-gray-400 justify-center font-semibold">
      { items.map((data, id) => (
        ...
        >
          <span className={data?.type === selected ? 'text-black flex items-center' : 'flex items-center'}>
            {data?.icon && <div className=''> {data?.type === selected ? data?.icon?.active : data?.icon?.InActive}</div>}
            {data?.text}
           </span>
