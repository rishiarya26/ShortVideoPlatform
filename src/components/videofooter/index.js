...
function VideoFooter({
  ...
  const type = {
    profile: `${(canShop &&  !adCards?.monitisation) ? 'bottom-32' : window.deferredPrompt ? 'bottom-12' : 'bottom-2'} videoFooter absolute left-0 w-2/3 pr-4 flex text-white ml-2`,
    feed: `${saveLook ? (window.deferredPrompt ? ' bottom-28 ' : 'bottom-20') : ' bottom-56 '} videoFooter absolute left-0  flex text-white ml-2 w-2/3 pr-4`,
