...
function ProfilePlaylist({ router }) {
  ...
  const dataFetcher = () =>
    getPlaylistDetails({ playlistid, offset: offset, firstApiCall: true });
    ...
    >
      <>
        ...
        >
          ...
          >
            {items?.map((item) => (
              <SwiperSlide key={item.content_id} id={item.content_id}>
      ...
      {showAppBanner ? (
        <AppBanner notNowClick={notNowClick} videoId={activeVideoId} />
      ) : (
        ""
      )}
      <Landscape />
