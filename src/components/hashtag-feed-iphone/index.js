 ...
 try{   console.log("event to send", id, event)
  ...
  let videos = data?.data;
    ...
    useEffect(()=>{
      ...
      shop?.adData?.monitisation && shop?.adData?.monitisationCardArray?.length > 0 &&   shop?.adData?.monitisationCardArray?.map((data)=> { toTrackMixpanel('monetisationProductImp',{pageName:pageName, hashtagName: item},{content_id:videoId,productId:data?.card_id, productUrl:data?.product_url, brandName: getBrand(data?.product_url), campaignId: shop?.campaignId})});
    },[shop])
   /************************ */ 
